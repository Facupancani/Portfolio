...
function MediaThumb({ project }) {
    ...
    const handleMouseEnter = () => {
        setHovered(true);
        if (project.videoMode === 'hover' && videoRef.current) {
    ...
    if (project.mediaType === 'video') {
        ...
        } else if (project.videoMode === 'click') {
            return (
                <video
                    src={project.thumb}
                    className="item-thumbnail-img img-full block w-[40vw] h-[40vw] rounded-md shadow-lg object-cover cursor-pointer"
                    controls
                    playsInline
                />
            );
